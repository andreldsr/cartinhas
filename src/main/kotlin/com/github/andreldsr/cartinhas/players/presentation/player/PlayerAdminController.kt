...
class PlayerAdminController(private val playerService: PlayerService) {
    ...
    @Operation(summary = "Create player", tags = ["Players"], security = [SecurityRequirement(name = "bearer-key")])
    fun create(@RequestBody request: CreatePlayerRequest) = playerService.create(request)

    @PutMapping("/{id}")
    @Operation(summary = "Update player", tags = ["Players"], security = [SecurityRequirement(name = "bearer-key")])
    fun update(@PathVariable id: UUID, @RequestBody player: Player) = playerService.update(id, player)

    @DeleteMapping("/{id}")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    @Operation(summary = "Delete player", tags = ["Players"], security = [SecurityRequirement(name = "bearer-key")])
